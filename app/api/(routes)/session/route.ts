import { checkForExistingSession, createSession, deleteSession, getSessionById } from "@/handlers/session";
...
export async function POST(req: NextRequest) {
  ...
  const token = sign({ sessionId: session.sessionId }, ENV.JWT_SECRET, { expiresIn: "72h" });
  try {
    cookieStore.set(ENV.COOKIE_NAME, serialize(ENV.COOKIE_NAME, token, {
      httpOnly: true,
    ...
    try {
      ...
      return NextResponse.json({ error: "Failed to delete invalid session from database" }, { status: 500 });
    }
  }
}
...
export async function GET(req: NextRequest) {
  ...
  if (!token) {
    ...
    try {
      ...
      return NextResponse.json({ sessionId: (decoded as { sessionId: string }).sessionId }, { status: 200 });
    } catch (e) {
