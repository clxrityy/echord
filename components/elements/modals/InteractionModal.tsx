...
export function InteractionModal({ onClose, open, interactionId, children, ...props }: InteractionModalProps) {
      ...
      <Dialog {...props} open={open} onClose={onClose}>
        {
          loading ? (
            ...
            <div className="flex flex-col items-start justify-start w-full gap-2">
              <h2 className="text-lg font-semibold">{interaction?.user.username}</h2>
              <p className="text-gray-600">{interaction?.eAlbum ? interaction.eAlbum.title : interaction?.eTrack && interaction.eTrack.title}</p>
            </div>
